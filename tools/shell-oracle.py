...
rows=json.loads((root/'tools/shell-cases.json').read_text())
version=subprocess.check_output([str(binary),'--version'],text=True).strip()
for row in rows:
    args=[str(binary),'--format','json','--no-color','--config',os.devnull]
    for registry in row.get('config',{}).get('trustedRegistries',[]):args.extend(['--trusted-registry',registry])
...
(root/'evidence/shell-reference.json').write_text(json.dumps(out,indent=2)+'\n',encoding='utf8')
